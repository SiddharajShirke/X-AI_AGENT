...
def test_database_seeds_profile_ten_contexts_and_ten_schedules(repository):
...
def test_profile_update_creates_version_snapshot(repository):
    before = repository.current_config_version()

    updated = repository.update_profile({"domain": "Privacy-preserving developer tools"})

    assert updated.domain == "Privacy-preserving developer tools"
    assert repository.current_config_version() == before + 1
    versions = repository.list_config_versions(limit=2)
    assert versions[0]["version"] == before + 1
    assert "Privacy-preserving developer tools" in versions[0]["snapshot_json"]


def test_repository_persists_draft_and_preference(repository):
    ...
    assert repository.get_draft(draft.id).text == draft.text
    assert preference.weight == 1.0
    assert repository.list_preferences()[0].rule.startswith("Prefer concrete")
